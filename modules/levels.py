...
class Levels(commands.Cog):
    ...
    async def on_ready(self) -> None:
        print(f"Levels module is now running.")
    ...
    async def on_message(self, message: discord.Message) -> None:
        if message.author == self.bot.user:
            return
        try:
            if not self.in_database(message.author.id):
                self.add_user(message.author.id)    
                return
            
            db.cursor.execute(f"SELECT exp, level FROM levels WHERE user_id={message.author.id}")
        ...
        if self.should_lvl_up(exp, level):
            exp = 0; level += 1
            await message.channel.send(f"{message.author.mention} just splashed to level {level}! Tidetastic!")
        
        db.cursor.execute(f"UPDATE levels SET exp={exp}, level={level} WHERE user_id={message.author.id}")
    ...
    async def rank(self, ctx: commands.Context, *args):
        async with ctx.channel.typing():
            ...
            try:
                ...
                if not self.in_database(user_id):
                    await ctx.send(f"Couldn't find '{user.name}' in my database.")
                    return
                
                record = db.cursor.execute(f"SELECT * FROM levels WHERE user_id={user_id}")
                record = record.fetchone()

                img: Image.Image = await self.generate_image(user, record[1], record[2], 
    ...
    async def reset(self, ctx: commands.Context, *args):
        async with ctx.channel.typing():
            try:
                ...
                db.cursor.execute(f"""UPDATE levels 
                                      SET bg_color={cfg.ranking.default_bg_color}, 
                ...
                self.delete_custom_background(f"{self.path}\\bg_{user_id}.png")
    ...
    def in_database(self, user_id: int) -> bool:
        db.cursor.execute(f"SELECT EXISTS(SELECT 1 FROM levels WHERE user_id={user_id})")
        ...
        try:
            ...
            for param in params.split(';'):
                ...
                match name:
                    ...
                    case "bg_image":
                        image_background = 1
                        img_path = f"{self.path}\\bg_{ctx.author.id}.png"
                        
                        if value in ("None", "none"):
                            image_background = 0
                            self.delete_custom_background(img_path)
                            
                        elif bool(value):
                            img = Image.open(requests.get(value, stream=True).raw)
                            self.resize_and_save(img, img_path)
            ...
            db.cursor.execute(f"""UPDATE levels 
                                  SET {', '.join([f'{key}={val}' for key, val in parsed.items()])}
    ...
    def resize_and_save(self, img: Image.Image, img_path: str):
        img = img.resize((cfg.ranking.img_width, cfg.ranking.img_height), Resampling.BICUBIC)
        img.save(img_path)


    def delete_custom_background(self, img_path: str):
    ...
    def add_user(self, user_id: int):
        try:
            db.cursor.execute(f"""INSERT INTO levels 
                                  VALUES ({user_id}, 1, 1, {cfg.ranking.default_bg_color}, 
    ...
    async def generate_image(self, user: discord.User, exp: int, level: int, 
        ...
        image = None
        if bool(bg_image):
            image = Image.open(f"{self.path}\\bg_{user.id}.png")
